...
class MatcherTest < Test::Unit::TestCase
  ...
  def test_different_lengths_rejected
    assert !Matcher.match(@numbers, @numbers + [1], 100, 5)
    assert !Matcher.match(@numbers + [1], @numbers, 100, 5)
  end
  ...
  def test_variation_within_delta_accepted
    assert Matcher.match(@numbers, [12, 55, 25, 100], 100, 5)
  end

  def test_clipped_variation_within_delta_accepted
    assert Matcher.match(@numbers, [12, 55, 25, 110], 100, 5)
  end

  def test_variation_greater_than_delta_rejected
    assert !Matcher.match(@numbers, [10, 60, 30, 98], 100, 5)
  end
end
